...
# these two are not really needed
...
# now we have created 2 objects: y, response, trt, treatement group
...
# simcost is a vector of length n, the overal cost for each individual

set.seed(1234) 
for(i in 1:2) # two treatment arms, 
# creating a correlation between y, outcome, and cost.
# outcome is sepsis, length of stay in hospital is correlated with cost.
{
...
# GENERATE 4 DATASETS WITH 4 COLUMNS: ID, RESPONSE, COST, TREATMENT
dataset1 <- data.frame(cbind(paste0("ID", c(1:dim(Outcome)[1])), Outcome[,1], Costs[,1], id))
...
#---------------------------------------------------------------------------------------
# GENERATE FIRST STATISTIC 'ICER' - function that generate 'ICER', 1 ICER for each dataset
# INPUT: 2 datasets 
# OUTPUT 9 elements: for each dataset 'mean' and 'se' across treatment and 'icer'
...
incCosts     = meanCostsB-meanCostsA      # DIFFERENCE BETWEEN TREATMENT ARM MEANS
incOutcome   = meanOutcomeA-meanOutcomeB  # DIFFERENCE BETWEEN TREATMENT ARM MEANS
icer         = incCosts/incOutcome        

# COMBINES THE TWO TREATEMENTS ARM TO GIVE YOU A NET BENEFIT STATISTIC FOR EACH DATASET
# AND COMPARES THE TWO DATASETS AT EACH WILLINGNESS TO PAY THRESHOLD.

# NUMBER OF DATASETS TO COMPARE
nDatasets            = ncol(Costs) 
# COVARIANCE BETWEEN TREATMENT ARM A 
covCostsA            = matrix(NA,nrow=nDatasets,ncol=nDatasets)
# CORRELATION BETWEEN TREATMENT ARM A
...
# DIFFERENCE BETWEEN COSTS FOR DATASETS
covDeltaCosts        = matrix(NA,nrow=nDatasets,ncol=nDatasets)

wtp                  = c(0:100)*5000 # Willingness To Pay threshold
NB                   = matrix(NA,nrow=length(wtp),ncol=nDatasets)
seNB                 = matrix(NA,nrow=length(wtp),ncol=nDatasets)
lclNB                = matrix(NA,nrow=length(wtp),ncol=nDatasets) # LOWER CONFIDENCE LIMIT
uclNB                = matrix(NA,nrow=length(wtp),ncol=nDatasets) # UPPER CONFIDENCE LIMIT
...
# MEAN CONCORDANCE CORRELATION COEFFICIENT
...
# PERCENTAGE OF LOSS OF ESTIMATE YOU ARE WILLING TO ACCEPT
...
# WE STOPPED HERE ON SATURDAY 13TH 2016 

# Covariance between two net benefit functions
#-------------------------------------------------------------------------------
" k is dataset2 m is datset2"
for(k in 1:nDatasets){
  for(m in 1:nDatasets){
    # COVARIANCE OF COSTS BETWEEN DATASET1 DATASET2 FOR TREATMENT A
    covCostsA[k,m]     = cor(Costs[id=="A",k],Costs[id=="A",m])*seCostsA[k]*seCostsA[m]
    # COVARIANCE OF COSTS BETWEEN DATASET1 DATASET2 FOR TREATMENT B
    covCostsB[k,m]     = cor(Costs[id=="B",k],Costs[id=="B",m])*seCostsB[k]*seCostsB[m]
    # SUM OF THE THE TWO ABOVE
    covDeltaCosts[k,m] = covCostsA[k,m] + covCostsB[k,m] 
    
    for(j in 1:length(wtp)){
      # COVARIANCE OF OUTCOME AT SPECIFIC WILLINGNESS TO PAY (OUTCOME IS TRANSFORMED TO MONEY) BETWEEN DATASET1 DATASET2 FOR TREATMENT A
      covOutcomeA[k,m,j]          = wtp[j]*wtp[j]*cor(Outcome[id=="A",k],Outcome[id=="A",m])*seOutcomeA[k]*seOutcomeA[m] 
      
      # COVARIANCE OF OUTCOME AT SPECIFIC WILLINGNESS TO PAY (OUTCOME IS TRANSFORMED TO MONEY) BETWEEN DATASET1 DATASET2 FOR TREATMENT B
      
      covOutcomeB[k,m,j]          = wtp[j]*wtp[j]*cor(Outcome[id=="B",k],Outcome[id=="B",m])*seOutcomeB[k]*seOutcomeB[m] 
      
      # COVARIANCE BETWEEN COST AND OUTCOME AT SPECIFIC WILLINGNESS TO PAY BETWEEN DATASET1 DATASET2 FOR TREATMENT A
      ...
      # DELTA FOR THE ABOVE TWO (DELTA FOR COST WAS CALCULATED BEFORE)
...
# NET BENEFIT: YOU SET A TO 0 AND COMPARE B TO 0; IF POSITIVE THEN TREATMENT B IS BETTER 
#-----------------------------------------------------------
...
for(k in 1:nDatasets){
  ...
  for(j in 1:length(wtp)){
    ...
    # OUTPUT GENERATE A TABLE OF 4 COLUMNS AS OUTPUT: NB, seNB, lclNB, uclNB
    
    for(m in 1:nDatasets){
      # test of difference (section 2.1)
      #------------------------------------------------------------------------------------------------------------------------
      # DIFFERENCE IN NET BENEFIT BETWEEN TREATMENTS IN DATASET1 AT WILLINGNESS TO PAY J
      # DIFFERENCE IN NET BENEFIT BETWEEN TREATMENTS IN DATASET1 AT WILLINGNESS TO PAY J
      ...
      # OUTPUT A TABLE WITH: diffNB, sediffNB, lcldiffNB, ucldiffNB, pvalueNB      
  
      # Lin's concordance correlation (section 2.3) 
      #---------------------------------------------------------------------------------------------------------------------------
      # ABSOLUTE VALUES OF NET DIFFERENCE
      mucccNB[k,m,j] = abs(diffNB[k,m,j])/sqrt(seNB[j,k]^2 * seNB[j,m]^2)
      
      # A DIFFERENT WAY OF EXPRESSING LIN CONCORDENCE: cbNB is a weighted standard difference which reduces concordence
      # accuracy comes from pearson correlation and precision comes from cbNB.
      # FOR PERFECT CONCORDENCE cbNB=1 AND rhoNB=1
      ...
      # TRANSFORMATION OF CONCORDENCE CORRELATION COEFFICIENT TO CONTINUOUS SCALE
      ...
      #------------------------------------------------------------------------------------------------------------------------------
      ccc01               = 0.4 # LOSS OF CORRELATION YOU ARE WILLING TO ACCEPT (ABOVE THERE IS AGREEMENT, NO OTHERWISE)
      pvaluecccNB1[k,m,j] = pnorm(-abs(zcccNB[k,m,j]-0.5*log((1+ccc01)/(1-ccc01)))/secccNB[k,m,j])
      # LOSS OF CORRELATION
      dLoss[k,m,j]        = (0.15*rhoNB[k,m,j])^2
      
      # LOSS OF CORRELATION YOU ARE WILLING TO ACCEPT (ABOVE THERE IS AGREEMENT, NO OTHERWISE)
